...
public class GSRRunManagerDerankConfirmScreen extends Screen {
    ...
    public GSRRunManagerDerankConfirmScreen(Screen parent, Runnable onConfirm) {
        super(Text.literal("Participant Change Will Invalidate Run"));
        this.parent = parent;
    ...
    protected void init() {
        ...
        addDrawableChild(ButtonWidget.builder(GSRButtonParameters.literal(GSRButtonParameters.RUN_MANAGER_DERANK_CONFIRM_APPLY), btn -> confirm())
                .dimensions(centerX - buttonWidth - gap / 2, y, buttonWidth, buttonHeight).build());
        addDrawableChild(ButtonWidget.builder(GSRButtonParameters.literal(GSRButtonParameters.RUN_MANAGER_DERANK_CONFIRM_CANCEL), btn -> cancel())
